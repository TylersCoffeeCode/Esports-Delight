    ...
    return (
        <div className="bg-[#0B0C13] w-full h-[100vh] flex flex-wrap items-start justify-center">
            <div className="w-4/5 h-[55%] mt-[5rem] flex flex-wrap">
                ...
                <div className=" bg-[rgba(0,0,0,0.8)] w-1/3 h-4/5 flex flex-wrap justify-center border-2 border-white" onClick={()=>navigate(`/article/${singleArticle.id}`)}>
                    ...
                    <Link to={`/article/${singleArticle.id}`} className=" text-blue-300 w-full text-right"><p>Read More →</p></Link>
                </div>
                <div className="h-[91%] w-3/5 ml-6 flex flex-wrap border-2 content-between">
                    {articles.slice(1, 5).map((article) => (
                        <div className="w-full h-1/4 flex flex-wrap content-end border-white border-2 cursor-pointer" onClick={()=>navigate(`/article/${article.id}`)}
                        style={{
                            backgroundImage:
                                `url(${article.image})`,
            ...
            <div className=" w-4/5 h-1/3 flex flex-wrap">
                ...
                <div className="w-full h-3/4 flex flex-wrap justify-around items-start  content-start overflow-y-scroll bg-[rgb(34,42,51)]">
                        {articles.slice(5).map((article) => (
                            <div className=" w-[47%] bg-[rgb(95,132,168)] h-16 my-2 flex items-center justify-between mt-6" onClick={()=>navigate(`/article/${article.id}`)}>
